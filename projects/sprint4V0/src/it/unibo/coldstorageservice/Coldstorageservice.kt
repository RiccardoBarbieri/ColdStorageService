...
class Coldstorageservice(name: String, scope: CoroutineScope, isconfined: Boolean = false) :
    ActorBasicFsm(name, scope, confined = isconfined) {
    ...
    override fun getBody(): (ActorBasicFsm.() -> Unit) {
        ...
        return { //this:ActionBasciFsm
            state("chargeFailed") { //this:State
                action { //it:State
                    if (checkMsgContent(
                            Term.createTerm("chargefailedtt(FW)"), Term.createTerm("chargefailedtt(FW)"),
            ...
            state("depositFailed") { //this:State
                action { //it:State
                    if (checkMsgContent(
                            Term.createTerm("chargedeposited(FW)"), Term.createTerm("chargedepfailed(FW)"),
                            currentMsg.msgContent()
                        )
                    ) { //set msgArgList
            ...
            state("forwardUpdate") { //this:State
                action { //it:State
                    CommUtils.outcyan(
                        "$name in ${currentState.stateName} | $currentMsg | ${
                            Thread.currentThread().getName()
                        } n=${Thread.activeCount()}"
                    )

                    if (checkMsgContent(
                            Term.createTerm("coapUpdate(RESOURCE,VALUE)"), Term.createTerm("coapUpdate(RES,VAL)"),
                            currentMsg.msgContent()
                        )
                    ) { //set msgArgList
                        updateResourceRep(
            ...
            state("waiting") { //this:State
                ...
                interrupthandle(
                    edgeName = "t049",
                    targetState = "forwardUpdate",
                    cond = whenDispatch("coapUpdate"),
                    interruptedStateTransitions
                )
            }
            state("checkAvailability") { //this:State
                action { //it:State
                    if (checkMsgContent(
                            Term.createTerm("storerequest(FW)"), Term.createTerm("storerequest(FW)"),
                            currentMsg.msgContent()
                        )
                    ) { //set msgArgList
                ...
                transition(edgeName = "goto", targetState = "requestTicket", cond = doswitchGuarded({
                    accepted
                }))
                transition(edgeName = "goto", targetState = "waiting", cond = doswitchGuarded({
                    !(accepted
                            )
                }))
            }
            ...
            state("replyTicket") { //this:State
                action { //it:State
                    if (checkMsgContent(
                            Term.createTerm("ticket(TICKET)"), Term.createTerm("ticket(TICKET)"),
                            currentMsg.msgContent()
                        )
                    ) { //set msgArgList
            ...
            state("requestDeposit") { //this:State
                action { //it:State
                    if (checkMsgContent(
                            Term.createTerm("initdeposit(TICKET)"), Term.createTerm("initdeposit(TICKET)"),
                            currentMsg.msgContent()
                        )
                    ) { //set msgArgList
            ...
            state("chargeTakenTT") { //this:State
                action { //it:State
                    if (checkMsgContent(
                            Term.createTerm("chargetakentt(FW)"), Term.createTerm("chargetakentt(FW)"),
                            currentMsg.msgContent()
                        )
                    ) { //set msgArgList
            ...
            state("chargeDeposited") { //this:State
                action { //it:State
                    if (checkMsgContent(
                            Term.createTerm("chargedeposited(FW)"), Term.createTerm("chargedeposited(FW)"),
                            currentMsg.msgContent()
                        )
                    ) { //set msgArgList
                    ...
                    updateResourceRep(
                        "actualcurrentcoldroom($ActualCurrentColdRoom)"
                    )
                    //genTimer( actor, state )
                }
                //After Lenzi Aug2002
